...
class App(Tk):
    ...
    def __init__(self, model, data_loader, gt_bboxes):
        ...
        frame2 = Frame(self)
        frame2.pack()
        self.GT_bool, self.BB_bool, self.points_bool, self.scale, self.overlay = IntVar(), IntVar(), IntVar(), IntVar(), IntVar()
    ...
    def visualize(self):
        ...
        #spec = fig.add_gridspec(3, 3, wspace=0.0, hspace=0.0, top=0.95, bottom=0.05, left=0.17, right=0.845)
        ...
        if self.head_fusion not in ("all", "gradcam"):
            ...
            if self.scale.get():  
                plt.colorbar(attmap, ax=ax_attn)
