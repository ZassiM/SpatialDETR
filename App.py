...
class App(Tk):
    ...
    def __init__(self, model, data_loader):
        ...
        Radiobutton(frame1, text = self.head_types[0].capitalize(), variable = self.selected_head_fusion, value = self.head_types[0]).grid(row=0,column=0)
        Radiobutton(frame1, text = self.head_types[1].capitalize(), variable = self.selected_head_fusion, value = self.head_types[1]).grid(row=0,column=1)
        Radiobutton(frame1, text = self.head_types[2].capitalize(), variable = self.selected_head_fusion, value = self.head_types[2]).grid(row=0,column=2)
        Radiobutton(frame1, text = "All", variable = self.selected_head_fusion, value = "all").grid(row=0,column=3)
        ...
        plot_button = Button(master = self, command = self.visualize, text = "Visualize")
    ...
    def get_all_attentions(self):
        ...
        self.labels = outputs[0]["pts_bbox"]['labels_3d'][self.thr_idxs]
    ...
    def visualize(self):
        ...
        if self.old_bbox_idx != self.selected_bbox.get():
            self.old_bbox_idx = self.selected_bbox.get()
            self.imgs_bbox = []
